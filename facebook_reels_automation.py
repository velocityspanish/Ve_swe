"""
Facebook Reels Automation - Bilingual English/Swedish Content Generator
IMPROVED VERSION: Better backgrounds, English categories, no repeats, Velocity Swedish branding
"""
...
# Swedish translations for display
CATEGORIES_SWEDISH = {
    ...
    "Wisdom": "Visdom",
    "Happiness": "Lycka",
    "Self Improvement": "Självutveckling",
    "Gratitude": "Tacksamhet",
    "Friendship": "Vänskap",
    "Hope": "Hopp",
    "Creativity": "Kreativitet",
    "Inner Peace": "Inre Frus",
...
def generate_phrases(category_english: str, num_phrases: int = 5) -> list:
    ...
    for attempt in range(max_attempts):
        try:
            ...
            prompt = f"""Create {num_phrases * 2} unique {category_english} phrases for English speakers learning Swedish.
...
2. Swedish translation (with commas matching the rhythm)
...
IMPORTANT: Create FRESH, UNIQUE phrases that haven't been used before."""
            ...
            payload = {
                "model": AI_MODEL,
                "messages": [
                    {"role": "system", "content": "You are a Swedish teacher. Create short, natural phrases with pauses."},
...
def get_fresh_fallback_phrases(category: str, num_phrases: int) -> list:
    ...
    all_fallbacks = {
        "Motivation": [
            {"english": "Believe in yourself.", "swedish": "Tro på dig själv.", "pronunciation": "Troo poh dig sylv."},
            {"english": "You are capable of amazing things.", "swedish": "Du klarar av fantastiska saker.", "pronunciation": "Doo klarar av fan-tas-tis-ka sa-ker."},
            {"english": "Dream big, start small.", "swedish": "Dröm stort, börja smått.", "pronunciation": "Druhm stort, bur-ya smott."},
            {"english": "Your future is created by your actions.", "swedish": "Din framtid skapas av dina handlingar.", "pronunciation": "Deen fram-teed ska-pas av dee-na hand-ling-ar."},
            {"english": "Never give up on your dreams.", "swedish": "Ge aldrig upp dina drömmar.", "pronunciation": "Yeh al-dree oop dee-na druhm-mar."},
        ],
        "Love": [
            {"english": "Love yourself first.", "swedish": "Älska dig själv först.", "pronunciation": "El-ska dig sylv furst."},
            {"english": "Love makes everything possible.", "swedish": "Kärlek gör allt möjligt.", "pronunciation": "Shär-lek gur alt muh-jligt."},
        ],
    }
...
def generate_all_audio(phrases: list, output_dir: str):
    ...
    for i, phrase in enumerate(phrases):
        ...
        # Generate Swedish audio
        sv_success = asyncio.run(generate_single_audio(phrase["swedish"], SWEDISH_VOICE, str(swedish_file)))
        if sv_success:
            print(f"    ✓ Swedish: {swedish_file.name}")
        else:
            cmd = ["ffmpeg", "-y", "-f", "lavfi", "-i", "anullsrc=r=24000:cl=mono", "-t", "2", str(swedish_file)]
            subprocess.run(cmd, capture_output=True)

        # Get ACTUAL durations
        en_duration = get_audio_duration(str(english_file))
        sv_duration = get_audio_duration(str(swedish_file))

        # Add pause between English and Swedish
        pause_between = 0.5
        ...
        if result.returncode != 0:
            ...
            with open(concat_file, "w", encoding="utf-8") as f:
                ...
                f.write(f"file '{swedish_file.as_posix()}'\n")
...
def generate_complete_image(phrase_data: dict, category_english: str, output_path: str):
    ...
    swedish = phrase_data.get("swedish", "")
    ...
    # Swedish text
    swedish_y = english_y + total_height + 110  # Increased from 100
    swedish_lines = wrap_text(swedish, font_large, VIDEO_WIDTH - 140)
    total_height = len(swedish_lines) * 95  # Increased from 75

    draw.rectangle(
        [(60, swedish_y - 55), (VIDEO_WIDTH - 60, swedish_y + total_height + 15)],
        fill=(80, 30, 30, 220)
    )

    for i, line in enumerate(swedish_lines):
        y_pos = swedish_y + (i * 95)  # Increased spacing
        draw.text(
            (VIDEO_WIDTH // 2, y_pos),
            line,
            fill=(255, 255, 0),
    ...
    pronunciation_y = swedish_y + total_height + 90  # Increased from 80
...
def generate_reel(category_english: str = None):
    ...
    print(f"\n{'='*80}")
    print(f"Category: {category_english} ({CATEGORIES_SWEDISH[category_english]})")
    print(f"{'='*80}\n")
    ...
    for i, phrase in enumerate(phrases, 1):
        print(f"  {i}. {phrase['english']} → {phrase['swedish']}")
    ...
    print("\n[3/4] Generating audio (English + Swedish with 500ms pause)...")
    ...
    print(f"  🏷️  Branding: Velocity Swedish")
    print(f"{'='*80}\n")
...
if __name__ == "__main__":
    print("\n" + "="*80)
    print("🇸🇪 VELOCITY SWEDISH - FACEBOOK REELS AUTOMATION 🇸🇪")
    print("="*80)
    print("\n✨ IMPROVED FEATURES:")
    ...
    print("  ✓ Velocity Swedish branding at bottom")
    ...
    for i, cat in enumerate(CATEGORIES_ENGLISH, 1):
        print(f"   {i:2d}. {cat} ({CATEGORIES_SWEDISH[cat]})")
    print(f"\n📅 DAILY CAPACITY:")
